...
<html lang="en" style="height: auto;"><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
  ...
  <script>
        ...
        function KiemTraNgay(){
            ...
            var today = new Date();
            today.setDate(today.getDate()-4015);
            if(day>=today){
        ...
        txtngay.blur(KiemTraNgay);
            ...
            function ktdc()
            {
                var regten=/^([A-Z|Đ]{1}[a-z|á|à|ả|ã|ạ|ă|ắ|ặ|ằ|ẳ|ẵ|â|ấ|ầ|ẩ|ẫ|ậ|é|è|ẻ|ẽ|ẹ|ê|ế|ề|ể|ễ|ệ|í|ì|ỉ|ĩ|ị|ó|ò|ỏ|õ|ọ|ô|ố|ồ|ổ|ỗ|ộ|ơ|ớ|ờ|ở|ỡ|ợ|ú|ù|ủ|ũ|ụ|ư|ứ|ừ|ử|ữ|ự|ý|ỳ|ỷ|ỹ|ỵ]{1,7})([A-Z|Ý|Đ]{1}[á|à|ả|ã|ạ|ă|ắ|ặ|ằ|ẳ|ẵ|â|ấ|ầ|ẩ|ẫ|ậ|é|è|ẻ|ẽ|ẹ|ê|ế|ề|ể|ễ|ệ|í|ì|ỉ|ĩ|ị|ó|ò|ỏ|õ|ọ|ô|ố|ồ|ổ|ỗ|ộ|ơ|ớ|ờ|ở|ỡ|ợ|ú|ù|ủ|ũ|ụ|ư|ứ|ừ|ử|ữ|ự|ý|ỳ|ỷ|ỹ|ỵ|a-z]*)$/;
                var ten=$("#form-diachi").val();
                if(regten.test(ten))
                {
                    $("#dc").html("*")
                return true;
                ...
                {
                    $("#dc").html("Sai định dạng")
                    return false;
                }
            }
            $("#form-diachi").blur(ktdc)
            //------------//
            function ktemail()
            {
          ...
          <section class="col-lg-12 connectedSortable ui-sortable">
            ...
            <div class="card">
              ...
              <form action="" method="post">
                <caption><strong style="color: deepskyblue; padding-top: 40px;padding-bottom: 40px; font-size: 30px; font-family: 'Times New Roman', Times, serif; text-indent: 20px;">Thông tin người dùng</strong></caption><br>
                <div>
                    <input type="text" name="form-manguoidung" id="form-manguoidung" style="height: 50px; min-width: 400px; margin-top: 40px; border: 3px double navy; border-top-style: hidden; border-left-style: hidden; border-right-style: hidden; font-family: 'Times New Roman', Times, serif;  text-align: center;" readonly placeholder="Mã người dùng" value="<?php if($layvt=2){echo $p->laycot("SELECT MaQLDT FROM quanlydaotao WHERE MaQLDT = '$layid' LIMIT 1");}if($layvt=3){echo $p->laycot("SELECT MaGV FROM giangvien WHERE MaGV = '$layid' LIMIT 1");}if($layvt=4){echo $p->laycot("SELECT MaHV FROM hocvien WHERE MaHV = '$layid' LIMIT 1");}?>"><br>
                    <div id="mand" style="color: red;"></div>
                </div>
                <div>
                    <input type="text" name="form-tennguoidung" style="height: 50px; min-width: 400px; margin-top: 40px; border: 3px double navy; border-top-style: hidden; border-left-style: hidden; border-right-style: hidden; font-family: 'Times New Roman', Times, serif;  text-align: center;"id="form-tennguoidung" placeholder="Họ và tên" value="<?php if($layvt=2){echo $p->laycot("SELECT TenQLDT FROM quanlydaotao WHERE MaQLDT = '$layid' LIMIT 1");}if($layvt=3){echo $p->laycot("SELECT TenGV FROM giangvien WHERE MaGV = '$layid' LIMIT 1");}if($layvt=4){echo $p->laycot("SELECT TenHV FROM hocvien WHERE MaHV = '$layid' LIMIT 1");}?>"><br>
                    <div id="ten" style="color: red;"></div>
                <div>
                    <input type="date" name="form-ngaysinh" style="height: 40px; min-width: 400px; margin-top: 40px; border: 3px double navy; border-top-style: hidden; border-left-style: hidden; border-right-style: hidden; font-family: 'Times New Roman', Times, serif;  text-align: center;" id="form-ngaysinh" placeholder="Ngày sinh" value="<?php if($layvt=2){echo $p->laycot("SELECT NgaySinh FROM quanlydaotao WHERE MaQLDT = '$layid' LIMIT 1");}if($layvt=3){echo $p->laycot("SELECT NgaySinh FROM giangvien WHERE MaGV = '$layid' LIMIT 1");}if($layvt=4){echo $p->laycot("SELECT NgaySinh FROM hocvien WHERE MaHV = '$layid' LIMIT 1");}?>"><br>
                    <div id="ns" style="color: red;"></div>  
                </div>
                <div>
                    <input type="text" name="form-email" style="height: 40px; min-width: 400px; margin-top: 40px; border: 3px double navy; border-top-style: hidden; border-left-style: hidden; border-right-style: hidden; font-family: 'Times New Roman', Times, serif;  text-align: center;" id="form-email" placeholder="Email" value="<?php if($layvt=2){echo $p->laycot("SELECT Email FROM quanlydaotao WHERE MaQLDT = '$layid' LIMIT 1");}if($layvt=3){echo $p->laycot("SELECT Email FROM giangvien WHERE MaGV = '$layid' LIMIT 1");}if($layvt=4){echo $p->laycot("SELECT Email FROM hocvien WHERE MaHV = '$layid' LIMIT 1");}?>"><br>
                    <div id="email" style="color: red;"></div>
                </div>
                <div>
                    <input type="text" name="form-diachi" style="height: 40px; min-width: 400px; margin-top: 40px; border: 3px double navy; border-top-style: hidden; border-left-style: hidden; border-right-style: hidden; font-family: 'Times New Roman', Times, serif;  text-align: center;" id="form-diachi" placeholder="Địa chỉ" value="<?php if($layvt=2){echo $p->laycot("SELECT DiaChi FROM quanlydaotao WHERE MaQLDT = '$layid' LIMIT 1");}if($layvt=3){echo $p->laycot("SELECT DiaChi FROM giangvien WHERE MaGV = '$layid' LIMIT 1");}if($layvt=4){echo $p->laycot("SELECT DiaChi FROM hocvien WHERE MaHV = '$layid' LIMIT 1");}?>"><br>
                    <div id="dc" style="color: red;"></div>
                </div>
                <input type="submit" value="Xác nhận sửa" id="btn-suathongtin" name="btn-suathongtin" class="btn btn-primary float-left">
                <input type="submit" value="Xóa người dùng" id="btn-xoa" name="btn-suathongtin" class="btn btn-primary float-right">
            </form>
            <?php
                    switch($_POST['btn-suathongtin'] ?? null)
                        {
                            case 'Xác nhận sửa':
                                {
                                    $matk = $_REQUEST['form-manguoidung'];
                                    ...
                                    $ngaysinh = $_REQUEST['form-ngaysinh'];
                                    $email = $_REQUEST['form-email'];
                                    $diachi = $_REQUEST['form-diachi'];
                                    if($matk != '' && $tennguoidung != '' && $ngaysinh != '' && $email != '' && $diachi != '')
                                    {
                                        if(substr($layid,0,2) == 'QL'){
                                            if($p->themxoasua("UPDATE quanlydaotao SET MaQLDT = '$matk',TenQLDT = '$tennguoidung',Ngaysinh = '$ngaysinh',Email = '$email',Diachi = '$diachi' WHERE MaQLDT = '$layid' LIMIT 1") == 1)
                                            {
                                                echo '<script> alert("Cập nhật thông tin QLDT thành công");</script>';
                                                header( "refresh:0;url=Quanlythongtinnguoidung.php" );
                                            }
                                            else
                                            {
                                            ...
                                            if(substr($layid,0,2) == 'GV'){
                                                if($p->themxoasua("UPDATE giangvien SET MaGV = '$matk',TenGV = '$tennguoidung',Ngaysinh = '$ngaysinh',Email = '$email',Diachi = '$diachi' WHERE MaGV = '$layid' LIMIT 1") == 1)
                                                {
                                                    echo '<script> alert("Cập nhật thông tin GV thành công");</script>';
                                                    header( "refresh:0;url=Quanlythongtinnguoidung.php" );
                                                }
                                                else
                                                {
                                                ...
                                                if(substr($layid,0,2) == 'HV'){
                                                    if($p->themxoasua("UPDATE hocvien SET MaHV = '$matk',TenHV = '$tennguoidung',Ngaysinh = '$ngaysinh',Email = '$email',Diachi = '$diachi' WHERE MaHV = '$layid' LIMIT 1") == 1)
                                                    {
                                                        echo '<script> alert("Cập nhật thông tin HV thành công");</script>';
                                                        header( "refresh:0;url=Quanlythongtinnguoidung.php" );
                                                    }
                                                    else
                                                    {
                            ...
                            case 'Xóa người dùng':
                                {
                                    $id_xoa = $_REQUEST['form-manguoidung'];
                                    if($id_xoa > 0)
                                    {
                                        if(substr($layid,0,2) == 'QL'){
                                            if($p->themxoasua("DELETE FROM quanlydaotao WHERE MaQLDT = '$id_xoa' LIMIT 1") == 1)
                                            {
                                                echo '<script> alert("Xóa người dùng Quản Lý Đào Tạo thành công");</script>';
                                                header( "refresh:0;url=Quanlythongtinnguoidung.php" );
                                            }
                                            else
                                            {
                                                echo '<script> alert("Xóa người dùng Quản Lý Đào Tạo không thành công");</script>';
                                            }
                                        }
                                            if(substr($layid,0,2) == 'GV'){
                                                if($p->themxoasua("DELETE FROM giangvien WHERE MaGV = '$id_xoa' LIMIT 1") == 1)
                                                {
                                                    echo '<script> alert("Xóa người dùng Giảng Viên thành công");</script>';
                                                    header( "refresh:0;url=Quanlythongtinnguoidung.php" );
                                                }
                                                else
                                                {
                                                    echo '<script> alert("Xóa người dùng Giảng Viên không thành công");</script>';
                                                }
                                            }
                                                if(substr($layid,0,2) == 'HV'){
                                                    if($p->themxoasua("DELETE FROM hocvien WHERE MaHV = '$id_xoa' LIMIT 1") == 1)
                                                    {
                                                        echo '<script> alert("Xóa người dùng Học Viên thành công");</script>';
                                                        header( "refresh:0;url=Quanlythongtinnguoidung.php" );
                                                    }
                                                    else
                                                    {
                                                        echo '<script> alert("Xóa người dùng Học Viên không thành công");</script>';
                                                    }
                                                }
                                    }
                                    else
                                    {
                                        echo '<script> alert("Vui lòng chọn người dùng cần xóa");</script>';
                                    }
                                    break;
                                }
                        }
