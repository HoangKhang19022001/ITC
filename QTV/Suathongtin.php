  ...
  <script>
        ...
        $(document).ready(function()
        {
            function ktmatk()
                ...
                function pass()
                {
                    var regten=/^([0-9A-Za-z]{1,10})$/;
                    var ten=$("#form-matkhau").val();
                    if(regten.test(ten))
                    {
                        $("#mk").html("*")
                    return true;
                    ...
                    {
                        $("#mk").html("Sai định dạng")
                        return false;
                    }
                }
                $("#form-matkhau").blur(pass)
                //------------//
                ...
                $("#btn-themnguoidung").click(function()
                    {
                        ...
                        if(pass()==false) 
                        {
                        alert("Pass không Hợp Lệ! Vui lòng nhập lại");
                        return false;
                        }
          ...
          <section class="col-lg-12 connectedSortable ui-sortable">
            ...
            <div class="card">
              ...
              <form action="" method="post">
                <caption><strong style="color: deepskyblue; padding-top: 40px;padding-bottom: 40px; font-size: 30px; font-family: 'Times New Roman', Times, serif; text-indent: 20px;">Thông tin tài khoản người dùng</strong></caption><br>
                <div>
                <input type="text"style="height: 40px; min-width: 400px; margin-top: 40px; border: 3px double navy; border-top-style: hidden; border-left-style: hidden; border-right-style: hidden; font-family: 'Times New Roman', Times, serif; text-align: center;" name="form-manguoidung" id="form-manguoidung" readonly placeholder="(GV001,HV001,QLDT001)ADMIN001" value="<?php echo $p->laycot("SELECT MaTK FROM taikhoan WHERE MaTK = '$layid' LIMIT 1");?>"><br>
                <div id="mand" style="color: red;"></div>
                </div>
                <div>
                <input type="text" style="height: 40px; min-width: 400px; margin-top: 40px; border: 3px double navy; border-top-style: hidden; border-left-style: hidden; border-right-style: hidden; font-family: 'Times New Roman', Times, serif; text-align: center;" name="form-tennguoidung" id="form-tennguoidung" placeholder="(GV001_,HV001_,QLDT001_)ADM001_Khang" value="<?php echo $p->laycot("SELECT TenDangNhap FROM taikhoan WHERE MaTK = '$layid' LIMIT 1");?>"><br>
                ...
                <select name="form-vaitro" id="form-vaitro" style="height: 40px; min-width: 400px; margin-top: 40px; border: 3px double navy; border-top-style: hidden; border-left-style: hidden; border-right-style: hidden; font-family: 'Times New Roman', Times, serif; text-align: center;">
                    <option value="<?php echo $p->laycot("SELECT PhanQuyen FROM taikhoan WHERE MaTK = '$layid' LIMIT 1");?>"><?php echo $p->laycot("SELECT VaiTro FROM taikhoan WHERE MaTK = '$layid' LIMIT 1");?></option>
                ...
                <div>
                <input type="text" style="height: 40px; min-width: 400px; margin-top: 40px; border: 3px double navy; border-top-style: hidden; border-left-style: hidden; border-right-style: hidden; font-family: 'Times New Roman', Times, serif; text-align: center;" name="form-matkhau" id="form-matkhau" placeholder="Mật Khẩu" value="<?php $reponse = $p->laycot("SELECT MatKhau FROM taikhoan WHERE MaTK = '$layid' LIMIT 1");$hash = $reponse;
                                        $hash_type = "md5";
                                        $email = "khangnguyenkl2001@gmail.com";
                                        $code = "a41c8a8876acd36e";
                                        $matkhau = file_get_contents("http://md5decrypt.net/Api/api.php?hash=".$hash."&hash_type=".$hash_type."&email=".$email."&code=".$code);echo $matkhau; ?>"><br>
                <div id="mk"style="color: red;"></div>
                </div>
                            ...
                            case 'Xác nhận sửa':
                                {
                                    ...
                                    $reponse = $_REQUEST['form-matkhau'];
                                        $hash = $reponse;
                                        $hash_type = "md5";
                                        $email = "khangnguyenkl2001@gmail.com";
                                        $code = "a41c8a8876acd36e";
                                        $matkhau = file_get_contents("http://md5decrypt.net/Api/api.php?hash=".$hash."&hash_type=".$hash_type."&email=".$email."&code=".$code);
                                    if($matk != '' && $tennguoidung != '' && $vaitro != '' && $matkhau != '')
                                    {
                                        if($p->themxoasua("UPDATE taikhoan SET MaTK = '$matk',TenDangNhap = '$tennguoidung',MatKhau = '$matkhau',PhanQuyen = '$vaitro',VaiTro = '$phanquyen', Matkhaumahoa = '$matkhaumahoa' WHERE MaTK = '$matk' LIMIT 1") == 1)
                                        {
                                            echo '<script> alert("Cập nhật thông tin thành công");</script>';
                                            header( "refresh:0;url=Quanlynguoidung.php" );
                                        }
                                        else
                                        {
