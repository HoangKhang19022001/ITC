            ...
            <div class="card">
              ...
              <div class="card-body" style="text-align: center;">
                <form action="" method="post">
                    <caption><strong style="color: deepskyblue; padding-top: 40px;padding-bottom: 40px; font-size: 30px; font-family: 'Times New Roman', Times, serif; text-indent: 20px;">Thêm lịch thi</strong></caption><br>
                    <input type="text"  name="form-malichthi" id="form-malichthi" placeholder="Mã Lịch Thi. VD: LT0001" style="text-align: center; height: 40px; min-width: 400px; margin-top: 40px; border: 3px double navy; border-top-style: hidden; border-left-style: hidden; border-right-style: hidden;"><br>
                    <?php
                        $p->load_selectionLH("SELECT * FROM lophoc");
                    ?><br>
                    <?php
                        $p->load_selectionPHLT("SELECT * FROM phonghoc");
                    ?><br>
                    <input type="time" id="form-giothi" name="form-giothi" title="Giờ Thi" style="text-align: center; height: 40px; min-width: 400px; margin-top: 40px; border: 3px double navy; border-top-style: hidden; border-left-style: hidden; border-right-style: hidden;"><br>
                    <input type="date" id="form-ngaythi" name="form-ngaythi" title="Ngày Thi" style="text-align: center; height: 40px; min-width: 400px; margin-top: 40px; border: 3px double navy; border-top-style: hidden; border-left-style: hidden; border-right-style: hidden;"><br>
                    <input type="number" id="form-siso" name="form-siso" placeholder="Sỉ số" style="text-align: center; height: 40px; min-width: 400px; margin-top: 40px; border: 3px double navy; border-top-style: hidden; border-left-style: hidden; border-right-style: hidden;"><br>
                    <input type="submit" value="Thêm lịch thi" id="btn-suathongtin" name="btn-suathongtin" class="btn btn-primary float-left">
                ...
                <?php
                    			switch($_POST['btn-suathongtin'] ?? null)
                                {
                                    case 'Thêm lịch thi':
                                    {
                                        $malt = $_REQUEST['form-malichthi'];
                                        $tenlh = $_REQUEST['form-tenlophoc'];
                                        $siso = $_REQUEST['form-siso'];
                                        $phongthi = $_REQUEST['form-phonghoc'];
                                        $giothi = $_REQUEST['form-giothi'];
                                        $ngaythi = $_REQUEST['form-ngaythi'];
                                        if($malt != '' && $tenlh != '' && $siso != '' && $phongthi != '' && $giothi != '' && $ngaythi != '')
                                        {
                                            if($p->themxoasua("INSERT INTO lichthi(MaLT,TenLopHoc,PhongThi,GioThi,NgayThi,SoLuongThiSinh) VALUES('$malt','$tenlh','$phongthi','$giothi','$ngaythi','$siso')")==1)
                                            {
                                                echo '<script> alert("Thêm lịch thi thành công");</script>';
                                                header( "refresh:0;url=Xeplichthi.php" );
                                            }
                                            else
                                            {
                                                echo '<script> alert("Thêm lịch thi không thành công. Vui lòng kiểm tra lại thông tin lich thi");</script>';
                                            }
                                        }
                                        else
                                        {
                                            echo '<script> alert("Vui lòng nhập đầy đủ thông tin lịch thi");</script>';
                                        }
                                        break;
                                    }
