          ...
          <section class="col-lg-12 connectedSortable ui-sortable">
            ...
            <div class="card">
              ...
              <form action="" method="post">
                <caption><strong style="color: deepskyblue; padding-top: 40px;padding-bottom: 40px; font-size: 30px; font-family: 'Times New Roman', Times, serif; text-indent: 20px;">Sửa thông tin lớp học</strong></caption><br>
                <input type="text" name="form-malophoc" style="height: 40px; min-width: 400px; margin-top: 40px; border: 3px double navy; border-top-style: hidden; border-left-style: hidden; border-right-style: hidden; font-family: 'Times New Roman', Times, serif; text-align: center;" id="form-malophoc" readonly placeholder="Mã Lớp Học" value="<?php echo $p->laycot("SELECT MaLH FROM lophoc WHERE MaLH = '$layid' LIMIT 1"); ?>"><br>
                <input type="text" name="form-tenlophoc" style="height: 40px; min-width: 400px; margin-top: 40px; border: 3px double navy; border-top-style: hidden; border-left-style: hidden; border-right-style: hidden; font-family: 'Times New Roman', Times, serif; text-align: center;" id="form-tenlophoc" placeholder="Tên Lớp Học" value="<?php echo $p->laycot("SELECT TenLopHoc FROM lophoc WHERE MaLH = '$layid' LIMIT 1");?>"><br>
                <input type="number" name="form-siso" style="height: 40px; min-width: 400px; margin-top: 40px; border: 3px double navy; border-top-style: hidden; border-left-style: hidden; border-right-style: hidden; font-family: 'Times New Roman', Times, serif; text-align: center;" id="form-siso" placeholder="Sỉ Số Lớp" value="<?php echo $p->laycot("SELECT Siso FROM lophoc WHERE MaLH = '$layid' LIMIT 1");?>"><br>
                <select name="form-cahoc" id="form-cahoc" style="height: 40px; min-width: 400px; margin-top: 40px; border: 3px double navy; border-top-style: hidden; border-left-style: hidden; border-right-style: hidden; font-family: 'Times New Roman', Times, serif; text-align: center;">
                    <option value="<?php echo $p->laycot("SELECT cahoc.Thoigian From lophoc,cahoc WHERE lophoc.MaCaHoc = cahoc.MaCaHoc AND lophoc.MaLH = '$layid' LIMIT 1;");?>"><?php echo $p->laycot("SELECT ThoiGian From lophoc,cahoc WHERE lophoc.MaCaHoc = cahoc.MaCaHoc AND lophoc.MaLH = '$layid' LIMIT 1;") ?></option>
                ...
                <input type="submit" value="Xóa Lớp Học" id="btn-xoa" name="btn-suathongtin" class="btn btn-primary float-right">
            </form>
                <?php
                    switch($_POST['btn-suathongtin'] ?? null)
                        {
                            case 'Xác nhận sửa':
                                {
                                    $malophoc = $_REQUEST['form-malophoc'];
                                    $tenlophoc = $_REQUEST['form-tenlophoc'];
                                    $siso = $_REQUEST['form-siso'];
                                    $cahoc = $_REQUEST['form-cahoc'];
                                    if($malophoc != '' && $tenlophoc != '' && $siso != '' && $cahoc != '')
                                    {
                                        if($p->themxoasua("UPDATE lophoc SET MaLH = '$malophoc',TenLopHoc = '$tenlophoc',Siso = '$siso',MaCaHoc = '$cahoc' WHERE MaLH = '$layid' LIMIT 1") == 1)
                                        {
                            ...
                            case 'Xóa Lớp Học':
                                {
                                    $id_xoa = $_REQUEST['form-malophoc'];
                                    if($id_xoa > 0)
                                    {
                                        if($p->themxoasua("DELETE FROM lophoc WHERE MaLH = '$layid' LIMIT 1") == 1)
                                        {
                                            echo '<script> alert("Xóa lớp học thành công");</script>';
                                        }
                                        else
                                        {
                                            echo '<script> alert("Xóa lớp học không thành công");</script>';
                                        }
                                    }
                                    else
                                    {
                                        echo '<script> alert("Vui lòng chọn lớp học dùng cần xóa");</script>';
                                    }
                                    break;
                                }
                        }
